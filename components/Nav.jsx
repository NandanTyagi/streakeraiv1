...
import { useSearchParams, pathname } from "next/navigation";
        ...
        <div className={`absolute right-6`}>
          ...
          >
            <ThreeDButton
              isSaved={isSaved}
              text="Dashboard"
              onClick={() => router.push(`${location.origin}/${pathname}/dashboard/${searchParams.toString()}`)}
              title="dashboard"
              isHistory={isHistory}
            >
              {" "}
              <span className="w-[80%] font-semibold">Dash</span>
              <span className="w-[20%] flex justify-center items-center">
              <DashboardIcon width={20} height={20} className="text-black rotate-90" />
              </span>
            </ThreeDButton>
          </div>
          <button
